...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    // Populate current_node vector with all the neighbors
    current_node->FindNeighbors();

    for (RouteModel::Node * neighbor : current_node->neighbors){
        // Set neighbours params
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    // Sort with compare function using multiple files
    // sort(RoutePlanner::open_list.begin(), RoutePlanner::open_list.end(), Compare);

    //Alternative sort method
    sort(open_list.begin(), open_list.end(), [](const auto & p1, const auto & p2)
	{
		return (p1->h_value + p1->g_value) > (p2->h_value + p2->g_value);
	});
    
    // Remove lower value node and return it
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // Reordering with for loop
    std::vector<RouteModel::Node> pf_correct_order;
    for(int i = path_found.size() - 1; i >= 0; i--){
        pf_correct_order.push_back(path_found[i]);
    }

    // Distance conversion and return path
    distance *= m_Model.MetricScale(); // Multiply the distance by the scale of the map to get meters.
    return pf_correct_order;
...
void RoutePlanner::AStarSearch() {
    ...
    // Initialise start_node, add it to open list
